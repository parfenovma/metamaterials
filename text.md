...
Цель работы - исследовать распространение звуковых волн в акустическом метаматериале с синусоидальными вырезами.
...
Для распространения возмущений в твердом теле имеем уравнение[7,8]:
...
Для учета поглощения волны в фотополимере, были взяты данные из работы по исследованию фотополимерного материала. С учетом демпфирования тензор напряжений модифицируется:
...
Интегрирование по времени осуществлялось с помощью неявного метода Ньюмарка (Newmark-$\beta$) с параметрами $\gamma = 0.5$ и $\beta = 0.25$. По сравнению с явными схемами (например, методом Рунге-Кутты 4-го порядка), данная схема является безусловно устойчивой и не вносит численного затухания, что критически важно для корректного анализа физического поглощения и групповой задержки, а также позволяет использовать более крупную сетку.

## Вычисление групповой задержки

В общем случае, мы ожидаем что импульс на выходе из метаматериала будет сильно искажен, поэтому вычисление групповой задержки по положению максимума выходного импульса может давать ошибочные результаты. Для находждения групповой задержки импульса произвольной формы обычно используется вейвлет-преобразование. Однако, в нашей задаче исходный импульс имеет простую форму. Поэтому, для определения групповой задержки при прохождении сигнала через метаматериал мы ищем точку с максимальной корреляцией между нормированными огибающими входного и выходного сигнала. 
...
Для вычисления поля напряжений внутри фотополимера был выбран пакет Gridap.jl. В отличие от COMSOL Multiphysics, данный пакет является программным обеспечением с открытым исходным кодом (open-source) и кроссплатформенным. Также он не имеет искусственных лицензионных ограничений на количество используемых процессорных ядер (в отличие от коммерческих пакетов) и легко масштабируется на вычислительные мощности. Это позволило использовать 6 потоков процессора с минимальными изменениями в коде. Кроме того, этот пакет легко интегрируется с нейросетями для будущих задач.
...
Весь вычислительный алгоритм, включая генерацию геометрии, построение сетки, решение дифференциальных уравнений и постобработку сигналов, был реализован на высокопроизводительном языке программирования Julia. 

Поскольку целью работы являлся расчёт отклика среды для множества комбинаций частот импульсов $f_0$ и амплитуд вырезов $A$ — задача была оптимизирована с помощью многопоточных вычислений. Вычисление каждой конфигурации является независимым, что позволило одновременно рассчитывать несколько конфигураций, задействуя несколько потоков процессора (см.рис. 2, архитектуру вычислений).
...
Кроме того, выбранный программный стек (язык `Julia`, сеткогенератор `Gmsh` и решатель `Gridap.jl`) обладает высокой производительностью и возможностью автоматического дифференцирования. Это открывает перспективы для интеграции созданной численной модели с алгоритмами машинного обучения. Планируется реализация алгоритма активного обучения (Active Learning) нейронной сети для решения сложной нелинейной обратной задачи: автоматического синтеза оптимальной геометрии образца по заданным целевым характеристикам — требуемой групповой задержке и ширине запрещенной зоны[9,10].

# Заключение

В ходе выполнения курсовой работы была полностью реализована и исследована модель распространения упругих волн в твердотельном метаматериале с синусоидальными вырезами. Основные результаты работы:

1) Обоснована актуальность создания твердотельных акустических метаматериалов для управления фазовой и групповой задержкой импульсов. Сформулирована фундаментальная проблема конверсии мод и сильной дифракции на разрывах границ. На основе этого определены цель и задачи исследования.
2) Сформирован математический базис модели: записаны уравнения динамической теории упругости для изотропного твердого тела, модифицированные с учетом параметров вязкоупругого демпфирования фотополимера. Сформулированы поглощающие граничные условия и закон возбуждения волнового пакета с окном Ханна. Выведена слабая форма уравнений для МКЭ, обоснован выбор неявной схемы Ньюмарка-$\beta$ и предложен математический аппарат вычисления групповой задержки через кросс-корреляцию аналитических огибающих сигналов с использованием преобразования Гильберта.
3) Осуществлена программная реализация двумерной параметрической геометрии в пакете `Gmsh` и алгоритма решения на языке Julia. Заданы экспериментально определенные физические константы фотополимера. Построена адаптивная сетка и проведена строгая проверка на сеточную сходимость. Разработана высокопроизводительная многопоточная архитектура кода с применением мьютексов для обеспечения потокобезопасности при генерации сеток.
4) С помощью пакета `ParaView` проведена пространственно-временная визуализация полей смещений, наглядно продемонстрировавшая эффекты обратного рассеяния и интерференции. Рассчитаны спектры энергетического пропускания и калибровочные кривые групповой задержки в зависимости от амплитуды вырезов $A$ (от 0.0 до 3.6 мм). Физически обосновано существование трех частотных режимов: длинноволнового, коротковолнового и соответствующего характерным размерам элемента.
* **В разделе дальнейших перспектив** предложен вектор развития работы, заключающийся в переходе к моделированию градиентной акустической мета-линзы на основе полученных калибровочных кривых. Описана перспективная схема интеграции вычислительного комплекса `Gridap.jl` с алгоритмами машинного обучения (Active Learning) для автоматического решения нелинейной обратной задачи синтеза геометрии по заданным акустическим характеристикам.
