module RunMatchingLayerFEMSweep
...
const DESIGN_ROOT = joinpath(PROJECT_ROOT, "tmp", "sinusoidal_material_lens_242khz")
const OUTPUT_ROOT = joinpath(PROJECT_ROOT, "tmp", "matching_layer_fem_sweep_242khz")

label(value) = replace(string(Float64(value)), "." => "p")

function direct_selected_cells()
    lines = readlines(joinpath(DESIGN_ROOT, "selected_polymer_direct.csv"))
...
function run_thickness(thickness_mm::Real)
    thickness_mm > 0 || throw(ArgumentError("this sweep contains positive layer thicknesses"))
    case_paths = paths(thickness_mm)
    cells = direct_selected_cells()
    ...
    rows = [
        let data=load(path)
            (
                ...
                local_peak_amplitude_m=data["local_peak_amplitude_m"],
                local_peak_x_mm=data["local_peak_x_mm"],
                local_peak_y_mm=data["local_peak_y_mm"],
            )
        end
        for path in files
    ]
    sort!(rows; by=row -> row.matching_thickness_mm)
    # Add the independently solved direct-interface point as t=0.
    direct_path = joinpath(
        PROJECT_ROOT,
        "tmp",
        "sinusoidal_material_lens_fem_242khz",
        "results",
        "polymer_direct_selected.jld2",
    )
    if isfile(direct_path)
        direct = load(direct_path)
        pushfirst!(rows, (
            matching_thickness_mm=0.0,
            focus_longitudinal_amplitude_m=direct["focus_longitudinal_amplitude_m"],
            local_peak_amplitude_m=direct["local_peak_amplitude_m"],
            local_peak_x_mm=direct["local_peak_x_mm"],
            local_peak_y_mm=direct["local_peak_y_mm"],
        ))
    end
    isempty(rows) && return nothing
    path = joinpath(OUTPUT_ROOT, "matching_layer_fem_sweep.csv")
    mkpath(dirname(path))
    open(path, "w") do io
...
function main(args=ARGS)
    ...
    elseif args == ["--summary"]
        ...
        error("usage: run_matching_layer_fem_sweep.jl --thickness=MM or --summary")
    end
end
