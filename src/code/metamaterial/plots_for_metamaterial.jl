...
begin
    ...
    if !isdir(SIG_DIR)
        ...
        md"Files found: $(length(files)). Amplitudes available (А): $unique_A"
    end
end
...
md"""
**Amplitude (A):** $(@bind sel_A Select(string.(unique_A))) mm
**Frequency:** $(@bind sel_F Select(string.(unique_F))) kHz
"""
...
begin
    if ismissing(data)
        md"**[!] No data for A=$sel_A, F=$sel_F**"
    else
        t_us = data["time"] .* 1e6
        
        p_in = plot(t_us, data["signal_in"], 
            title="Pressure of incoming impulse", ylabel="MPa", 
            color=:red, legend=false, lw=2, grid=true)
            
        p_out = plot(t_us, data["signal_out"], 
            title="Pressure of outgoing impulse", xlabel="time, microseconds", ylabel="MPa", 
