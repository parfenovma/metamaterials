...
Для численного моделирования распространения акустических волн в метаматериале применяется open-source фреймворк Gridap.jl на языке Julia, который обеспечивающий высокую производительность для решения уравнений в частных производных методом конечных элементов.
...
Для численного решения в пакете Gridap.jl задача формулируется в слабой (вариационной) форме. Задача заключается в поиске функции смещений $\mathbf{u} \in U$, такой, что для любой допустимой тестовой функции $\mathbf{v} \in V_0$ выполняется интегральное тождество:
...
Интегрирование по времени осуществлялось с помощью неявного метода Ньюмарка (Newmark-$\beta$) с параметрами $\gamma = 0.5$ и $\beta = 0.25$, что соответствует правилу трапеций. Данная схема является безусловно устойчивой и не вносит численного затухания, что критически важно для корректного анализа физического поглощения и групповой задержки.

## Как определяем групповую задержку

Для определения групповой задержки при прохождении сигнала через метаматериал мы ищем точку с максимальной корреляцией между нормированными огибающими входного и выходного сигнала
...
На длине образца укладывается ровно 2 пространственных периода. Варьируемым геометрическим параметром в исследовании выступала амплитуда выреза $A$, характеризующая глубину проникновения геометрии внутрь образца с каждой стороны. В процессе расчетов параметр $A$ изменялся в диапазоне от 0.0 до 3.0 мм.
...
Для генерации сетки использовалась программа Gmsh через интерфейс Julia. Для оптимизации вычислений, шаг сетки адаптивно изменялся в зависимости от частоты исследуемого импульса $f_0$, и в зависимости от геометрии. Вблизи границ, шаг сетки сделан более мелким ($h_{min} = 0,15$ мм) (см. рис.1, пример сетки).

![рис.1, пример сетки](../pic/mesh.png)

Использовались базисные функции Лагранжа второго порядка (квадратичные элементы), обеспечивающие высокую гладкость поля перемещений.
...
Визуализация распространения акустического импульса была проведена в программе `ParaView`. На рисунке 2 представлен снимок распределения модуля вектора смещений в один из моментов времени.

![Рис.2, пример визуализации в программе ParaView.](../pic/example.png)
...
![Рис.3, совмещенные графики задержки и пропускания](../pic/delay_and_transmission.svg)
